...
static const char* shm_name = "/read_only_shm";
int sig_pipefd[2];
int epollfd;
int listenfd;
int shmfd;
    ...
    while(!stop_child) {
        ...
        for (int i = 0; i < number; i++) {
            ...
            if((sockfd == connfd) && ( events[i].events & EPOLLIN )) {
                memset(share_mem + idx*BUFFSIZE, '\0', BUFFSIZE );
                ret = recv(connfd, share_mem + idx*BUFFSIZE, BUFFSIZE-1, 0);
                if(ret < 0) {
                    if(errno != EAGAIN) {
                        stop_child = true;
                    }
                ...
                if(ret < 0) {
                    ...
                    Send(connfd, share_mem + client * BUFFSIZE, BUFFSIZE, 0);
                }
            }
            else {
                continue;
                    ...
                    switch(signals[i] ) {
                      case SIGCHLD: {
                        ...
                        while ((pid = waitpid(-1, &stat, WNOHANG)) > 0) {
                          ...
                          sub_process[pid] = -1;
                          if((del_user < 0) || (del_user > LISTENQ)) {
